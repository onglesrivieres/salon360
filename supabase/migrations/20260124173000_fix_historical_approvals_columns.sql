/*
  # Fix Historical Approvals Column References

  ## Problem
  The historical approval functions have two issues:
  1. Reference `st.approved_by_id` but actual column is `approved_by`
  2. Reference `employees.name` but actual column is `display_name`

  ## Fix
  Update both functions to use correct column names:
  - `st.approved_by_id` → `st.approved_by`
  - `completed_by.name` → `completed_by.display_name`
  - `approved_by.name` → `approved_by.display_name`
*/
