/*
  # Remove Spa Expert Role

  The "Spa Expert" role has been removed from the business. No employees
  currently hold this role. This migration removes all DB references.

  ## Changes
  - Drop and recreate employees_role_valid constraint without 'Spa Expert'
  - Drop and recreate valid_role_name constraint on role_permissions without 'Spa Expert'
  - Recreate set_approval_deadline() without v_closer_is_spa_expert
  - Recreate approve_ticket() without v_is_spa_expert
  - Delete any leftover role_permissions rows for Spa Expert
*/

-- ============================================================================
-- A. employees_role_valid constraint — remove 'Spa Expert'
-- ============================================================================
...
ALTER TABLE public.employees ADD CONSTRAINT employees_role_valid
  CHECK (role <@ ARRAY['Technician','Trainee','Receptionist','Manager','Owner','Supervisor','Cashier','Admin']::text[]);

-- ============================================================================
-- B. valid_role_name constraint on role_permissions — remove 'Spa Expert'
-- ============================================================================
...
-- ============================================================================
-- C. set_approval_deadline() — remove v_closer_is_spa_expert
-- ============================================================================
...
-- ============================================================================
-- D. approve_ticket() — remove v_is_spa_expert
-- ============================================================================
...
DECLARE
  ...
  v_is_technician boolean; v_is_supervisor boolean;
...
BEGIN
  ...
  CASE v_ticket.approval_required_level
    ...
    WHEN 'technician' THEN
      IF v_is_technician OR v_is_receptionist OR v_is_supervisor THEN
...
-- ============================================================================
-- E. Delete any leftover role_permissions rows for Spa Expert
-- ============================================================================
DELETE FROM public.role_permissions WHERE role_name = 'Spa Expert';
