/*
  # Restore Correct Technician Function

  ## Summary
  Restores the correct `get_sorted_technicians_for_store` function that was
  accidentally overwritten by the security fixes migration.

  ## Problem
  Migration 20251129064932 (fix_security_issues_final) overwrote the correct
  function from 20251129062113 (exclude_cashiers_from_technician_functions).
  This caused technicians to disappear from the Ticket Editor because:
  - Missing required fields (legal_name, open_ticket_count, ready_at, estimated_completion_time)
  - Missing check-in logic (checked-in employees should appear regardless of schedule)
  - Missing employee_stores junction table check (multi-store support)
  - Using wrong service column (estimated_duration_minutes instead of duration_min)
  - Returning 'unavailable' instead of 'neutral' status

  ## Solution
  Restore the correct function with all required features:
  1. Proper return fields matching TypeScript interface
  2. Check-in status logic
  3. Weekly schedule filtering
  4. Multi-store support via employee_stores table
  5. Cashier exclusion
  6. Security improvements (immutable search_path)

  ## Changes
  1. Drop incorrect function version
  2. Recreate correct function with proper security settings
*/

-- Drop the incorrect function (accepts uuid, text)
...
-- Recreate the correct function with security improvements
...
BEGIN
  ...
  queue_positions AS (
    ...
    WHERE trq.store_id = p_store_id
      ...
      -- Include if: no schedule OR scheduled for today OR checked in today
      AND (
        e.weekly_schedule IS NULL
        OR COALESCE((e.weekly_schedule->v_day_name->>'is_working')::boolean, true) = true
        OR EXISTS (
