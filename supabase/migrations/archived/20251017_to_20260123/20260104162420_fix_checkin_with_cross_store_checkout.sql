/*
  # Fix Check-In with Cross-Store Auto-Checkout

  ## Overview
  Comprehensive fix for employee check-in functionality that addresses all known issues:
  - Removes conflicting UNIQUE constraint that prevents multiple sessions per day
  - Adds cross-store auto-checkout logic
  - Validates commission employees are excluded
  - Maintains 8:45 AM check-in time restriction
  - Preserves all security and validation requirements

  ## Key Changes

  1. Database Schema
     - Remove UNIQUE(employee_id, store_id, work_date) constraint
     - Add partial unique index to prevent simultaneous active check-ins at same store
     - Maintain support for multiple sessions per day after check-out

  2. Check-In Logic
     - Automatically check out from ANY other store before checking in
     - Block commission employees with clear error message
     - Validate 8:45 AM check-in time in store timezone
     - Check attendance_display is enabled
     - Support idempotent check-in (safe to call multiple times)
     - Initialize last_activity_time for daily employee tracking

  3. Security
     - All tables fully qualified with public schema
     - search_path set to empty string
     - Proper RLS policies maintained

  ## Impact
  - All employees can check in after 8:45 AM EST
  - Checking in at Store B auto-checks out from Store A
  - Multiple sessions per day supported after proper check-out
  - Commission employees properly blocked
  - Clear error messages for all validation failures
*/

-- ============================================================================
-- STEP 1: Remove Conflicting UNIQUE Constraint
-- ============================================================================

-- Drop the conflicting UNIQUE constraint if it exists
-- This constraint prevents multiple sessions per day even after check-out
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'attendance_records_employee_id_store_id_work_date_key'
  ) THEN
    ALTER TABLE public.attendance_records
    DROP CONSTRAINT attendance_records_employee_id_store_id_work_date_key;
  END IF;
END $$;

-- ============================================================================
-- STEP 2: Add Partial Unique Index for Active Check-Ins Only
-- ============================================================================

-- This index ensures an employee can only have one active (unchecked-out)
-- session per store per day, while allowing multiple completed sessions
DROP INDEX IF EXISTS idx_attendance_active_checkin_unique;

CREATE UNIQUE INDEX idx_attendance_active_checkin_unique
ON public.attendance_records (employee_id, store_id, work_date)
WHERE status = 'checked_in' AND check_out_time IS NULL;

COMMENT ON INDEX idx_attendance_active_checkin_unique IS
'Ensures employee can only have one active check-in per store per day. Allows multiple sessions after check-out.';

-- ============================================================================
-- STEP 3: Rebuild check_in_employee Function with Cross-Store Logic
-- ============================================================================

-- Drop all existing variants of the function
...
'Creates attendance record for employee check-in. Validates: check-in time >= 8:45 AM in store timezone, attendance_display enabled, not commission employee. Automatically checks out from other stores before checking in. Supports multiple sessions per day after check-out. All tables fully qualified for security.';

-- ============================================================================
-- STEP 4: Update check_out_employee Function
-- ============================================================================
...
'Updates attendance record for employee check-out. Validates: attendance_display enabled, not commission employee. Uses store configured timezone for date matching. All tables fully qualified for security.';

-- ============================================================================
-- STEP 5: Add Indexes for Performance
-- ============================================================================

-- Ensure we have optimal indexes for check-in queries
CREATE INDEX IF NOT EXISTS idx_attendance_records_employee_store_date
ON public.attendance_records(employee_id, store_id, work_date);

CREATE INDEX IF NOT EXISTS idx_attendance_records_employee_status
ON public.attendance_records(employee_id, status)
WHERE status = 'checked_in' AND check_out_time IS NULL;

COMMENT ON INDEX idx_attendance_records_employee_status IS
'Optimizes cross-store check-in queries to find active check-ins at other stores';
