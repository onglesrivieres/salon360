/*
  # Fix save_safe_balance_snapshot - Use Correct Constraint Name

  1. Problem
    - Function references `unique_store_date` constraint
    - Actual constraint name is `unique_safe_balance_store_date`
    - Causes function to fail with "constraint does not exist" error

  2. Solution
    - Update ON CONFLICT clause to use correct constraint name
    - `ON CONFLICT ON CONSTRAINT unique_safe_balance_store_date`

  3. Impact
    - Fixes backfill_safe_balance_snapshots function
    - Allows historical balance snapshots to be created successfully
*/
...
BEGIN
  ...
  -- Insert or update the snapshot using correct constraint name
...
Uses unique_safe_balance_store_date constraint in ON CONFLICT to avoid column ambiguity issues.';
