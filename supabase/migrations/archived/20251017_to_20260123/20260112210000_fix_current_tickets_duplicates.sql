/*
  # Fix Current Tickets Duplicate Rows Bug

  ## Problem
  current_tickets CTE joins sale_tickets with ticket_items (one-to-many),
  causing duplicate rows when an employee has multiple items on their ticket.
  This corrupts the position calculation - first ready shows as #4 instead of #1.

  ## Root Cause
  ticket_items is a one-to-many table. If a ticket has 3 service items,
  the JOIN returns 3 duplicate rows for that employee.

  ## Solution
  Add DISTINCT ON (employee_id) to current_tickets CTE to get one row per employee.
*/
...
BEGIN
  ...
  RETURN QUERY
  -- FIX: Use DISTINCT ON to get one row per employee (prevents duplicates from ticket_items)
  ...
  -- Use store_services for duration (not deprecated services table)
...
'Returns sorted technicians for queue display. Uses DISTINCT ON to prevent duplicate rows from ticket_items. Positions calculated for ready/small_service only.';
