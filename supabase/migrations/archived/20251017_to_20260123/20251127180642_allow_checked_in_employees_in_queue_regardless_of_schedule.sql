/*
  # Allow Checked-In Employees in Queue Regardless of Schedule

  ## Overview
  Updates the `get_sorted_technicians_for_store` function to display employees who are
  checked in and have joined the ready queue, even if they don't have a schedule for today.

  ## Problem
  - Employees without a schedule for today are excluded from the technician queue
  - Even when they check in and join the queue, they don't appear in the Ticket Editor
  - This prevents legitimate employees who show up to work from being assigned tickets

  ## Solution
  - Check if employee is checked in for the given date
  - Include employees who meet ANY of these conditions:
    1. Have no schedule configured (weekly_schedule IS NULL)
    2. Are scheduled for today (weekly_schedule shows true for current day)
    3. Are checked in today (attendance_records shows checked_in status)

  ## Changes
  1. Update queue_positions CTE to include checked-in employees
  2. Update main query WHERE clause to include checked-in employees
  3. Use EXISTS subquery to check attendance_records table

  ## Impact
  - Employees who check in can always join the queue
  - Schedule-based filtering still applies for non-checked-in employees
...
-- Recreate with checked-in employee logic
