/*
  # Fix Technician Queue Join - Remove work_date Filter

  ## Problem
  The get_sorted_technicians_for_store function references `q.work_date` when joining
  with the technician_ready_queue table, but this column doesn't exist in that table.

  The technician_ready_queue table only has:
  - employee_id
  - store_id
  - ready_at
  - status
  - current_open_ticket_id

  ## Solution
  Remove the work_date filter from the JOIN condition since the queue table doesn't
  track dates - it's a real-time queue where employees join/leave dynamically.

  ## Changes
  - Remove `AND q.work_date = p_date::date` from the JOIN clause
  - Keep only employee_id and store_id in the JOIN condition
*/

-- Drop and recreate with correct join conditions
