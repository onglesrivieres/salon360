/*
  # Add Weekly Schedule Filter to Technician Function

  1. Changes
    - Update `get_sorted_technicians_for_store` function to accept a date parameter
    - Filter employees based on their weekly_schedule configuration
    - Only show employees who are scheduled to work on the specified date
    - Employees with NULL weekly_schedule are available all days (backward compatibility)

  2. Purpose
    - Fix issue where employees not scheduled for a specific day still appear in ticket editor
    - Ensure technician selection respects weekly schedules configured in employee settings

  3. Implementation
    - Add p_date DATE parameter with default value of CURRENT_DATE
    - Calculate day of week from provided date
    - Map day number to day name (sunday, monday, tuesday, etc.)
    - Filter employees in both queue_positions CTE and main query
    - Check: weekly_schedule IS NULL OR weekly_schedule->>day_name = 'true'
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS get_sorted_technicians_for_store(uuid);

-- Recreate with date parameter and schedule filtering
