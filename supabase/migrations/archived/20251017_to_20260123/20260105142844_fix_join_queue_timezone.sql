/*
  # Fix Timezone Mismatch in join_ready_queue_with_checkin
  ...
  timezone. This caused a mismatch when the dates differ (e.g., late evening
  in EST when UTC is already the next day).
  ...
  - Changed v_today calculation to use store timezone via get_store_timezone()
*/
...
COMMENT ON FUNCTION public.join_ready_queue_with_checkin IS 'Adds employee to ready queue with attendance validation and cooldown check. Uses store timezone for date calculations. Returns position and total queue size.';
