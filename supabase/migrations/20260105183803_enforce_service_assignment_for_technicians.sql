/*
  # Enforce Service Assignment Requirement for Technician Queue

  ## Overview
  Updates the get_sorted_technicians_for_store function to enforce service assignment
  requirements. Removes traditional role-based filtering and relies exclusively on
  the employee_services junction table.

  ## Changes

  1. **Service Assignment Enforcement**
     - Remove OR condition that included traditional service-performing roles
     - Remove Cashier exclusion logic (no longer needed)
     - Only employees with entries in employee_services table will appear
     - Provides granular control over who can perform services

  2. **Simplified Logic**
     - Single source of truth: employee_services table
     - No more role-based assumptions
     - Explicit authorization required for each employee-service combination

  ## Important Notes
  - Employees must have service assignments in employee_services table to appear in queue
  - Store filtering continues to work via employee_stores junction table
  - Status, attendance, and schedule filtering remains unchanged
  - This change provides better control and eliminates implicit service permissions
*/

-- Drop and recreate get_sorted_technicians_for_store with service assignment enforcement
...
'Returns sorted technicians for the queue display. Only includes employees with explicit service assignments in employee_services table. Uses employee_stores for proper store filtering.';
