/*
  # Fix Timezone Regression in join_ready_queue_with_checkin

  ## Overview
  Fixes a timezone bug where `CURRENT_DATE` (UTC) was used to look up attendance
  records that store `work_date` in the store's local timezone (EST). After 7 PM EST,
  UTC rolls to the next calendar day, causing the attendance lookup to miss the
  employee's active check-in record and returning a false CHECK_IN_REQUIRED error.

  ## Root Cause
  The original fix (20260105142844_fix_join_queue_timezone.sql) used the timezone-aware
  calculation `(CURRENT_TIMESTAMP AT TIME ZONE store_tz)::date`. This was carried into
  squash_07. However, later migrations (20260204000004, 20260204000006, 20260204000007)
  rewrote the entire function using `CURRENT_DATE` (UTC), reintroducing the bug.

  ## Fix
  Replace `v_today := CURRENT_DATE` with:
    v_store_timezone := public.get_store_timezone(p_store_id);
    v_today := (CURRENT_TIMESTAMP AT TIME ZONE v_store_timezone)::date;

  This matches the pattern used by `check_in_employee` and the original Jan 5 fix.
*/

-- ============================================================================
-- FUNCTION: join_ready_queue_with_checkin (fix timezone regression)
-- ============================================================================
...
AS $$
DECLARE
  v_attendance record; v_today date; v_store_timezone text;
  v_all_completed boolean; v_ticket_id uuid;
...
BEGIN
  -- Use store timezone for date calculation (not UTC)
  v_store_timezone := public.get_store_timezone(p_store_id);
  v_today := (CURRENT_TIMESTAMP AT TIME ZONE v_store_timezone)::date;
