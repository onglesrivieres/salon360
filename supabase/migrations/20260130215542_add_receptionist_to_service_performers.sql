-- Allow Receptionist role to perform services (appear in technician dropdown,
-- be assignable on tickets). Adds 'Receptionist' to the role filter in
  ...
  filtered_technicians AS (
    ...
    WHERE LOWER(e.status) = 'active'
      AND (e.role @> ARRAY['Technician']::text[] OR e.role @> ARRAY['Supervisor']::text[]
           ...
           OR e.role @> ARRAY['Receptionist']::text[])
