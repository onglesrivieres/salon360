/*
  ...
  ## Problem
  The get_sorted_technicians_for_store function exists in two overloaded versions:
  - One with p_date as DATE type
  - One with p_date as text type

  This causes PostgREST to fail with error PGRST203: "Could not choose the best
  candidate function" when calling the RPC from the frontend.

  ## Solution
  1. Drop both overloaded versions explicitly by specifying their signatures
  2. Recreate the function with a single signature using text type for p_date
  3. This ensures only one version exists and eliminates ambiguity

  ## Changes
  - DROP both overloaded versions of get_sorted_technicians_for_store
  - CREATE single version with (uuid, text) signature
  - Function logic remains unchanged
*/

-- Drop both overloaded versions explicitly
...
-- Recreate the function with a single signature (text parameter)
...
'Returns sorted technicians for the queue display. Only includes employees with explicit service assignments in employee_services table. Uses employee_stores for proper store filtering. Single signature version to avoid overloading conflicts.';
