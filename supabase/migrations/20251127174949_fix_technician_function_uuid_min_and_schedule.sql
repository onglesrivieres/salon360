/*
  # Fix Technician Function - MIN(UUID) Error and Improve Schedule Logic

  1. Problem Identified
    - MIN(st.id) fails because MIN() doesn't support UUID type
    - Need to use MIN_BY or ORDER BY + LIMIT to get oldest ticket's ID
    - Weekly schedule filter is working but we need to handle it more gracefully

  2. Solution
    - Use array_agg with ORDER BY to get the first (oldest) ticket ID
    - Make weekly_schedule checking more robust with COALESCE
    - Keep the permissive NULL handling (NULL schedule = available all days)

  3. Changes
    - Replace MIN(st.id) with proper aggregation
    - Improve readability and maintainability
    - Ensure all technicians scheduled for today appear in the list
*/

-- Drop the existing function
DROP FUNCTION IF EXISTS get_sorted_technicians_for_store(uuid, DATE);

-- Recreate with fixed UUID aggregation and improved schedule logic
...
BEGIN
  ...
  WITH employee_open_tickets AS (
    SELECT
      ...
      -- Use array_agg with ORDER BY to get the oldest ticket's ID
  ...
  queue_positions AS (
    ...
    WHERE trq.store_id = p_store_id
      ...
      -- NULL schedule = available all days, otherwise check specific day
      AND (e.weekly_schedule IS NULL OR COALESCE((e.weekly_schedule->v_day_name)::boolean, true) = true)
  )
  SELECT
    e.id as employee_id,
    e.legal_name,
