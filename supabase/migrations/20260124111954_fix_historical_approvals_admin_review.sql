/*
  # Fix Historical Approvals Column Reference

  ## Overview
  Fix column reference from requires_supervisor_approval to requires_admin_review.

  ## Changes

  ### Functions
  - `get_historical_approvals_for_manager` - Fixed st.requires_supervisor_approval → st.requires_admin_review
  - `get_historical_approvals_for_supervisor` - Fixed st.requires_supervisor_approval → st.requires_admin_review

  ## Notes
  - The sale_tickets table uses `requires_admin_review`, not `requires_supervisor_approval`
  - The frontend interface expects `requires_supervisor_approval`, so we alias the column
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(st.requires_admin_review, false) AS requires_supervisor_approval,
  ...
  WHERE
    ...
    AND (
      st.requires_admin_review = false
      OR st.requires_admin_review IS NULL
    )
  GROUP BY
    st.id,
    st.ticket_no,
