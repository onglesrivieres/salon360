/*
  # Hide Employees with No Schedule from Ticket Editor

  ## Summary
  Updates the `get_sorted_technicians_for_store` function to hide employees
  who have no schedule configured (weekly_schedule IS NULL) unless they check in.

  ## Problem
  The previous migration allowed employees with no schedule to appear automatically.
  This is incorrect - employees should have an explicit schedule or check in to appear.

  ## Solution
  Change the COALESCE default from `true` back to `false` for the weekly_schedule check.
  This treats employees with no schedule as "not working" unless they explicitly check in.

  ## Behavior After This Fix
  1. Employees with no schedule (NULL): **HIDDEN** unless they check in
  ...
  ## Changes
  1. Change `COALESCE(..., true)` back to `COALESCE(..., false)` in queue_positions CTE
  2. Change `COALESCE(..., true)` back to `COALESCE(..., false)` in main SELECT WHERE clause
*/
