-- ============================================================================
-- Migration: Add Match Calculation Setting
-- Description: Add enable_match_calculation setting to allow per-ticket toggle
--              that calculates tax from subtotal only (ignoring discounts).
-- ============================================================================

-- ============================================================================
-- 1. Update add_missing_store_settings() to include match calculation setting
-- ============================================================================
...
BEGIN
  ...
  -- ========================================================================
  -- PAYMENT CATEGORY (11 settings — 6 existing + 3 tax + 1 match + 1 card options)
  -- ========================================================================
  ...
  -- Match calculation setting (between enable_tax and tax rates)
  INSERT INTO public.app_settings (store_id, setting_key, setting_value, default_value, category, display_name, description, display_order, dependencies)
  VALUES (p_store_id, 'enable_match_calculation', 'false', 'false', 'Payment', 'Match Calculation', 'Show Match button to calculate tax from subtotal only (ignoring discounts)', 75, '[{"key": "enable_tax", "type": "requires", "label": "Enable Tax"}]'::jsonb)
  ON CONFLICT (store_id, setting_key) DO NOTHING;
  IF FOUND THEN v_added_count := v_added_count + 1; END IF;
  ...
  -- Card options setting
  INSERT INTO public.app_settings (store_id, setting_key, setting_value, default_value, category, display_name, description, display_order, dependencies)
  VALUES (p_store_id, 'enable_card_options', 'false', 'false', 'Payment', 'Card Options', 'Show card type selection (Interac, Mastercard, Visa) on card payments', 95, '[{"key": "enable_card_payments", "type": "requires", "label": "Enable Card Payments"}]'::jsonb)
  ON CONFLICT (store_id, setting_key) DO NOTHING;
  IF FOUND THEN v_added_count := v_added_count + 1; END IF;
...
-- ============================================================================
-- 2. Insert match calculation setting for all existing stores
-- ============================================================================
