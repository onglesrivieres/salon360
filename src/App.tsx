...
const EndOfDayPage = lazyWithReload(() =>
  import("./pages/EndOfDayPage").then((m) => ({ default: m.EndOfDayPage })),
);
const SafeBalancePage = lazyWithReload(() =>
  import("./pages/SafeBalancePage").then((m) => ({
    default: m.SafeBalancePage,
  })),
);
const TipReportPage = lazyWithReload(() =>
  import("./pages/TipReportPage").then((m) => ({ default: m.TipReportPage })),
);
const AttendancePage = lazyWithReload(() =>
  import("./pages/AttendancePage").then((m) => ({ default: m.AttendancePage })),
);
const EmployeesPage = lazyWithReload(() =>
...
const ConfigurationPage = lazyWithReload(() =>
  import("./pages/ConfigurationPage").then((m) => ({
    default: m.ConfigurationPage,
  })),
);
const PendingApprovalsPage = lazyWithReload(() =>
...
const ResourcesPage = lazyWithReload(() =>
...
function AppContent() {
  ...
  // Redirect to Resources page on login if there are unread resources
  useEffect(() => {
    if (!isAuthenticated || !selectedStoreId || !session?.employee_id) return;
    if (hasCheckedUnreadRef.current) return;
    hasCheckedUnreadRef.current = true;

    (async () => {
      try {
        const { data, error } = await supabase.rpc(
          "get_unread_resources_count",
          {
        ...
        // Non-critical — silently fall through to default Tickets page
      }
    })();
  }, [isAuthenticated, selectedStoreId, session?.employee_id]);
  ...
  const [insightsCustomDateRange, setInsightsCustomDateRange] = useState<
    DateRange | undefined
  >();
  const [approvalsDate, setApprovalsDate] = useState(getCurrentDateEST());
  const [approvalsQueueStartDate, setApprovalsQueueStartDate] =
    useState(getCurrentDateEST());
  const [approvalsQueueEndDate, setApprovalsQueueEndDate] =
    useState(getCurrentDateEST());
  ...
  const makeDateHandler =
    (setter: (d: string) => void) => (newDate: string) => {
      if (
        session?.role_permission === "Cashier" ||
        session?.role_permission === "Receptionist"
      )
  ...
  const workingHoursCheck = useWorkingHoursCheck(
    selectedStoreId,
    session?.role_permission,
  );
  ...
  async function checkStoreAccess() {
    ...
    if (availableStores.length > 0) {
      const previouslySelectedStore =
        sessionStorage.getItem("selected_store_id");
      if (
        previouslySelectedStore &&
        availableStores.some((s) => s.id === previouslySelectedStore)
      ) {
      ...
      <HomePage
        onActionSelected={(
          ...
          if (action === "report" && session && storeId) {
            ...
            if (
              checkedInStoreId &&
              availableStoreIds?.includes(checkedInStoreId)
            ) {
      ...
      >
        <ChunkErrorBoundary>
          ...
          >
            ...
            {currentPage === "tipreport" && (
              <TipReportPage
                selectedDate={tipReportDate}
                onDateChange={handleTipReportDateChange}
              />
            )}
            {currentPage === "eod" && (
              <EndOfDayPage
                selectedDate={eodDate}
                onDateChange={handleEodDateChange}
              />
            )}
            {currentPage === "safebalance" && (
              <SafeBalancePage
                selectedDate={safeBalanceDate}
                onDateChange={handleSafeBalanceDateChange}
              />
            )}
            {currentPage === "attendance" && (
              <AttendancePage
                currentDate={attendanceDate}
                onCurrentDateChange={setAttendanceDate}
              />
            )}
