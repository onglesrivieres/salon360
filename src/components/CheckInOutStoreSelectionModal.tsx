...
interface CheckInOutStoreSelectionModalProps {
  isOpen: boolean;
  storeIds: string[];
  employeeId: string;
...
export function CheckInOutStoreSelectionModal({
  isOpen,
  storeIds,
  employeeId,
  onSelect,
  onClose
}: CheckInOutStoreSelectionModalProps) {
  const { t } = useAuth();
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Select Store for Check In/Out"
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          You have access to multiple stores. Please select which store you want to check in or out from.
        </p>
