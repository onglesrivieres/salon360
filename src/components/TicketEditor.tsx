...
import {
  formatDateTimeEST,
  convertToESTDatetimeString,
  convertESTDatetimeStringToUTC,
} from "../lib/timezone";
...
import {
  TicketPhotosSection,
  TicketPhotosSectionRef,
} from "./tickets/TicketPhotosSection";
...
export function TicketEditor({
  ...
  const enableGiftCardPayments = getSettingBoolean(
    "enable_gift_card_payments",
    true,
  );
  const enableMixedPayments = getSettingBoolean(
    "enable_mixed_payment_methods",
    true,
  );
  const requireCustomerName = getSettingBoolean(
    "require_customer_name_on_tickets",
    false,
  );
  const requireCustomerPhone = getSettingBoolean(
    "require_customer_phone_on_tickets",
    false,
  );
  const requireOpeningCashValidation = getSettingBoolean(
    "require_opening_cash_validation",
    false,
  );
  const showCustomerName = getSettingBoolean("show_customer_name_field", true);
  const showCustomerPhone = getSettingBoolean(
    "show_customer_phone_field",
    true,
  );
  const showTodaysColor = getSettingBoolean("show_todays_color_field", true);
  const requireTodaysColor = getSettingBoolean(
    "require_todays_color_on_tickets",
    false,
  );
  ...
  const isApproved =
    ...
    ticket?.approval_status === "auto_approved";
  ...
  const canVoid =
    session &&
    session.role_permission &&
    Permissions.tickets.canVoid(session.role_permission);
  ...
  const canEditNotes =
    ...
    (!ticketId ||
      (ticket &&
        Permissions.tickets.canEditNotes(
          session.role_permission,
          !!ticket.closed_at,
        )));
  ...
  const canClose =
    session &&
    session.role_permission &&
    Permissions.tickets.canClose(session.role_permission);

  const canMarkCompleted =
    session &&
    session.role_permission &&
    Permissions.tickets.canMarkCompleted(session.role_permission);

  const canSelectPaymentMethod =
    session &&
    session.role_permission &&
    Permissions.tickets.canSelectPaymentMethod(session.role_permission);

  const canViewPaymentDetailsWhenClosed =
    session &&
    session.role_permission &&
    Permissions.tickets.canViewPaymentDetailsWhenClosed(
      session.role_permission,
    );
  ...
  const canReopen =
    session &&
    session.role_permission &&
    Permissions.tickets.canReopen(session.role_permission);

  const canRequestChanges =
    session &&
    session.role_permission &&
    Permissions.tickets.canRequestChanges(session.role_permission);

  const canDelete =
    session &&
    session.role_permission &&
    Permissions.tickets.canDelete(session.role_permission);
  ...
  const isRestrictedCustomerInfoRole =
    session?.role &&
    (session.role.includes("Technician") || session.role.includes("Trainee"));

  const canViewFullPhoneWhenClosed =
    session?.role_permission &&
  ...
  useEffect(() => {
    if (clientLookup.client && !linkedClient) {
      ...
      if (!formData.customer_name && clientLookup.client.name) {
        setFormData((prev) => ({
          ...prev,
          customer_name: clientLookup.client!.name,
        }));
      }
      // Fetch blacklisted_by name if blacklisted
      if (
        clientLookup.client.is_blacklisted &&
        clientLookup.client.blacklisted_by
      ) {
  ...
  async function checkAttendanceAndValidate() {
    ...
    try {
      ...
      if (!data) {
        showToast(
          "You must be checked in to create tickets. Please check in first.",
          "error",
        );
    ...
    async function checkPendingReopenRequest() {
      ...
      try {
        const { data, error } = await supabase.rpc(
          "has_pending_ticket_reopen_request",
          {
      ...
      if (ticketId) {
        ...
        setFormData({
          ...
          payment_cash: firstItem
            ...
            : "0",
          payment_card: firstItem
            ? parseFloat(firstItem.payment_card || 0).toString()
            : "0",
          payment_gift_card: firstItem
            ? parseFloat(firstItem.payment_gift_card || 0).toString()
            : "0",
          tip_customer_cash: firstItem
            ? parseFloat(firstItem.tip_customer_cash || 0).toString()
            : "0",
          tip_customer_card: firstItem
            ? parseFloat(firstItem.tip_customer_card || 0).toString()
            : "0",
          ...
          discount_percentage: firstItem
            ...
            : "0",
          discount_amount: firstItem
            ? parseFloat(firstItem.discount_amount || 0).toString()
            : "0",
          discount_percentage_cash: firstItem
            ? parseFloat(firstItem.discount_percentage_cash || 0).toString()
            : "0",
          discount_amount_cash: firstItem
            ? parseFloat(firstItem.discount_amount_cash || 0).toString()
            : "0",
        ...
        setItems(
          ticketItems.map((item: any) => ({
            ...
            tip_customer: (
              parseFloat(item.tip_customer_cash || 0) +
  ...
  function calculateTotalTimerDuration(time: Date): number {
    return items.reduce(
      (sum, item) => sum + calculateServiceDuration(item, time),
      0,
    );
  }
  ...
  const hasHighAddonPrice = items.some(
    (item) => (parseFloat(item.addon_price) || 0) > 15,
  );
    ...
    setTempPaymentData({
      ...
      payment_cash:
        method === "Cash"
          ? amountStr
          : existingData
            ? formData.payment_cash || "0"
            : "",
      payment_card:
        method === "Card"
          ? amountStr
          : existingData
            ? formData.payment_card || "0"
            : "",
      ...
      discount_percentage: existingData
        ...
        : "",
      discount_percentage_cash: existingData
        ? formData.discount_percentage_cash || "0"
        : "",
  ...
  async function checkOpeningCashRecorded(
    storeId: string,
    ticketDate: string,
  ): Promise<boolean> {
    const { data, error } = await supabase
      ...
      .select(
        "opening_cash_amount, bill_20, bill_10, bill_5, bill_2, bill_1, coin_25, coin_10, coin_5",
      )
      .eq("store_id", storeId)
      .eq("date", ticketDate)
      .maybeSingle();
  ...
  async function handleSaveComment() {
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "updated",
        `${session?.display_name} added a comment`,
      );
  ...
  async function handleSave() {
    ...
    if (
      requireTodaysColor &&
      showTodaysColor &&
      (!formData.todays_color || formData.todays_color.trim() === "")
    ) {
    ...
    if (
      totalPayments > 0 &&
      Math.abs(totalPayments - subtotalAfterDiscount) > 0.01
    ) {
      const difference = subtotalAfterDiscount - totalPayments;
      if (difference > 0) {
        showToast(
          `Payment amount ($${totalPayments.toFixed(2)}) is less than service total ($${subtotalAfterDiscount.toFixed(2)}). Please adjust payment amounts.`,
          "error",
        );
      }
    }
    ...
    for (const item of items) {
      if (item.is_custom) {
        ...
        if (!canEmployeePerformService(item.employee_id, item.service_id)) {
          ...
          showToast(
            `${employee?.display_name || "This employee"} cannot perform ${service?.name || "this service"}.`,
            "error",
          );
          return;
        }
    ...
    try {
      ...
      if (!ticketId && selectedStoreId && requireOpeningCashValidation) {
        const openingCashRecorded = await checkOpeningCashRecorded(
          selectedStoreId,
          selectedDate,
        );
        if (!openingCashRecorded) {
          showToast(
            "Opening cash count must be recorded before creating tickets. Please go to End of Day and count the opening cash first.",
            "error",
          );
          setSaving(false);
          return;
      ...
      if (ticketId && ticket) {
        ...
        if (
          ...
          session?.role_permission &&
          Permissions.tickets.canReviewSelfServiceTicket(
            session.role_permission,
          )
        ) {
        ...
        if (
          ...
          formData.opening_time !== ticket.opened_at
        ) {
          updateData.opened_at = formData.opening_time;
          await logActivity(
            ticketId,
            "updated",
            `${session?.display_name} updated ticket opening time`,
            {
              opening_time: {
        ...
        await logActivity(
          ticketId,
          "updated",
          `${session?.display_name} updated ticket`,
          {
            customer_name: formData.customer_name,
            total,
            ...
            discount_percentage_cash:
              parseFloat(formData.discount_percentage_cash) || 0,
            discount_amount_cash:
              parseFloat(formData.discount_amount_cash) || 0,
          },
        );
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
          ...
          let discountPercentageCash =
            parseFloat(formData.discount_percentage_cash) || 0;
          let discountAmountCash =
            parseFloat(formData.discount_amount_cash) || 0;
          ...
          const itemData: any = {
            ...
            custom_service_name: item.is_custom
              ? item.custom_service_name
              : null,
            employee_id: item.employee_id,
        ...
        await logActivity(
          newTicket.id,
          "created",
          `${session?.display_name} created ticket`,
          {
            ticket_no: ticketNo,
            customer_name: formData.customer_name,
            total,
      ...
      if (photosSectionRef.current?.hasPendingChanges) {
        const photoUploadSuccess =
          await photosSectionRef.current.uploadPendingPhotos(
  ...
  async function handleSelectBusyTechnician(
    technicianId: string,
  ...
  function handleTechnicianSelect(
    technicianId: string,
  ...
  async function handleCloseTicket() {
    ...
    if (
      ...
      showToast(
        "Customer phone number is required to close this ticket",
        "error",
      );
      return;
    }
    ...
    if (
      ...
      !["Cash", "Card", "Mixed", "Gift Card"].includes(formData.payment_method)
    ) {
      showToast(
        "Please select a payment method before closing the ticket",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (
        linkedClient &&
        formData.todays_color &&
        formData.todays_color.trim()
      ) {
        try {
      ...
      await logActivity(
        ticketId,
        "closed",
        `${session?.display_name} closed ticket`,
        {
      ...
      showToast(
        "Ticket closed successfully. Approval workflow initiated.",
        "success",
      );
      onClose();
    } catch (error) {
  ...
  async function handleReopenTicket() {
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "updated",
        `${session?.display_name} reopened ticket`,
        {
          reopened: true,
        },
      );
  ...
  async function handleSubmitReopenRequest(data: {
    reason_comment: string;
    requested_changes_description: string;
  }) {
    if (!canRequestChanges) {
      showToast(
        "You do not have permission to request ticket changes",
        "error",
      );
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "updated",
        `${session?.display_name} requested changes to closed ticket`,
        {
  ...
  async function handleMarkCompleted() {
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "updated",
        `${session?.display_name} marked ticket as completed`,
        {
          completed_at: new Date().toISOString(),
        },
      );
  ...
  async function handleDeleteTicket() {
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "deleted",
        `${session?.display_name} deleted ticket`,
        {
  ...
  async function handleVoidTicket() {
    ...
    try {
      ...
      await logActivity(
        ticketId,
        "voided",
        `${session?.display_name} voided ticket`,
        {
          ticket_no: ticket.ticket_no,
          void_reason: voidReason.trim(),
        },
            ...
            {timeRemaining && (
              <span className="text-[10px] text-gray-500">
                {timeRemaining === "Expired"
                  ...
                  : `Auto-approves in ${timeRemaining}`}
              </span>
            )}
          </div>
        );
        ...
        <div className="fixed inset-0 md:right-0 md:left-auto md:top-0 h-full w-full md:max-w-4xl bg-white shadow-xl z-50 overflow-y-auto flex items-center justify-center">
          <div className="bg-white rounded-lg p-8 max-w-md mx-4 text-center">
            ...
            <p className="text-gray-600 mb-6">
              You do not have permission to create tickets. Only Admin and
              Receptionist roles can create new tickets.
            </p>
      ...
      <div className="fixed inset-0 md:right-0 md:left-auto md:top-0 h-full w-full md:max-w-2xl bg-white shadow-xl z-50 flex flex-col">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-3 md:px-4 py-3 md:py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onClose}
                className="px-3 py-2 text-sm bg-white border border-gray-300 text-gray-700 rounded hover:bg-gray-50 transition-colors font-medium min-h-[44px] md:min-h-0"
              >
                Close
              </button>
            </div>
        ...
        <div className="p-3 md:p-4 space-y-2 pb-4 flex-1 overflow-y-auto min-h-0">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-red-600 mt-1 font-medium">
                      This ticket requires admin review before any changes can
                      be made.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-amber-600 mt-1">
                    Voided on{" "}
                    {ticket?.voided_at
                      ? formatDateTimeEST(ticket.voided_at)
                      : "Unknown"}
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-blue-700 mt-1">
                      You cannot approve tickets you closed. The assigned
                      technician must approve it.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-green-700">
                    • You can only create new tickets for yourself
                    <br />
                    • You cannot edit, complete, or close tickets after creation
                    <br />• Only receptionists and managers can modify tickets
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-blue-700 mt-1">
                      You can only view this ticket. You cannot edit, complete,
                      or close it. Contact a receptionist or manager for any
                      changes.
                    </p>
                  </div>
                </div>
          ...
          <div className="border border-gray-200 rounded-lg p-2 bg-gray-50">
            {isEditingOpeningTime ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="datetime-local"
                  value={convertToESTDatetimeString(
                    formData.opening_time || new Date().toISOString(),
                  )}
                  onChange={(e) => {
                    const utcDateString = convertESTDatetimeStringToUTC(
                      e.target.value,
                    );
                    setFormData({ ...formData, opening_time: utcDateString });
                  }}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <p className="text-sm text-gray-900 font-medium flex-shrink-0">
                      {formData.opening_time
                        ? formatDateTimeEST(formData.opening_time)
                        : "Not set"}
                    </p>
                    {!isReadOnly &&
                      session &&
                ...
                {ticketId && items.length > 0 && (
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    >
                      {formatTimerDisplay(
                        calculateTotalTimerDuration(currentTime),
                      )}
                    </p>
                    {items.some(
                      (item: any) =>
                  ...
                  {showCustomerPhone && (
                    <div>
                      ...
                      <div className="relative">
                        {isRestrictedCustomerInfoRole ? (
                          <input
                            ...
                            value={
                              formData.customer_phone
                                ? maskPhoneNumber(formData.customer_phone)
                                : ""
                            }
                            disabled
                      ...
                      {!isRestrictedCustomerInfoRole &&
                        !shouldMaskPhone &&
                        showCustomerPhone &&
                        hasEnoughDigitsForLookup(formData.customer_phone) &&
                        !clientLookup.isLoading &&
                        !isTicketClosed &&
                        !isVoided && (
                          <div className="mt-1">
          ...
          <div className="border border-gray-200 rounded-lg p-3 bg-blue-50">
            ...
            {employees.length === 0 && !isTicketClosed && !isVoided && (
              <div className="mb-3 p-2 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-xs text-amber-800">
                  ...
                  {getFullDayName(getDayOfWeek(selectedDate))}. Please update
                  employee schedules.
                </p>
              </div>
            )}
            ...
            {!isTicketClosed && !isVoided ? (
              <TechnicianQueue
                sortedTechnicians={
                  session?.role &&
                  Permissions.tickets.canViewAllTechniciansInEditor(
                    session.role,
                  )
                    ? sortedTechnicians
                    : sortedTechnicians.filter(
                        (t) => t.employee_id === session?.employee_id,
                      )
                }
              ...
              <div className="flex flex-wrap gap-2">
                {items.length > 0 &&
                  Array.from(
                    new Set(items.map((item) => item.employee_id)),
                  ).map((employeeId) => {
                      ...
                      {services
                        ...
                        .filter((service) =>
                          canEmployeePerformService(
                            selectedTechnicianId || lastUsedEmployeeId,
                            service.service_id,
                          ),
                        )
                        .sort((a, b) =>
                          selectedCategory === "all" ? 0 : b.price - a.price,
                        )
                        ...
                        .map((service) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (!employeeId) {
                                showToast(
                                  "Please wait while loading employee data",
                                  "error",
                                );
                                return;
                              }
                  ...
                  >
                    ...
                    {ticketId && (item as any).started_at ? (
                      ...
                      >
                        ...
                        <span>
                          Timer:{" "}
                          {formatTimerDisplay(
                            calculateServiceDuration(item, currentTime),
                          )}
                        </span>
                        {!(item as any).timer_stopped_at &&
                          !(item as any).completed_at && (
                    ...
                    <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                updatedItems[index].custom_service_name =
                                  e.target.value;
                      ...
                      <div className="w-24">
                        ...
                        <div className="relative">
                          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-sm text-gray-500 z-10">
                            $
                          </span>
                          <NumericInput
                            step="0.01"
                ...
                <div className="grid grid-cols-3 gap-2 mb-2.5">
                  {enableCashPayments && (
                    ...
                    >
                      <div className="flex flex-col items-center gap-1.5">
                        <Banknote
                          className={`w-6 h-6 ${formData.payment_method === "Cash" ? "text-green-600" : "text-gray-600"}`}
                        />
                        <span
                          className={`text-xs font-semibold ${formData.payment_method === "Cash" ? "text-green-700" : "text-gray-700"}`}
                        >
                          Cash
                        </span>
                      </div>
                  ...
                  {enableCardPayments && (
                    ...
                    >
                      <div className="flex flex-col items-center gap-1.5">
                        <CreditCard
                          className={`w-6 h-6 ${formData.payment_method === "Card" ? "text-blue-600" : "text-gray-600"}`}
                        />
                        <span
                          className={`text-xs font-semibold ${formData.payment_method === "Card" ? "text-blue-700" : "text-gray-700"}`}
                        >
                          Card
                        </span>
                      </div>
                    ...
                    >
                      <div className="flex flex-col items-center gap-1.5">
                        <div className="flex gap-1">
                          <Banknote
                            className={`w-5 h-5 ${formData.payment_method === "Mixed" ? "text-teal-600" : "text-gray-600"}`}
                          />
                          <CreditCard
                            className={`w-5 h-5 ${formData.payment_method === "Mixed" ? "text-teal-600" : "text-gray-600"}`}
                          />
                        ...
                        <span
                          className={`text-xs font-semibold ${formData.payment_method === "Mixed" ? "text-teal-700" : "text-gray-700"}`}
                        >
                          Mixed
                        </span>
                      </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
              Notes / Comments
              {(hasRequiresPhotosService || hasHighAddonPrice) && !ticketId && (
            ...
            <textarea
              ...
              className={`w-full px-2 py-1.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                (hasRequiresPhotosService || hasHighAddonPrice) &&
                !ticketId &&
              ...
              placeholder={
                canEditNotes
                  ? (hasRequiresPhotosService || hasHighAddonPrice) && !ticketId
                    ? "Notes required for this service..."
          ...
          {/* Photos Section */}
          {(ticketId ||
            (!ticketId && (hasRequiresPhotosService || hasHighAddonPrice))) &&
            selectedStoreId && (
              <TicketPhotosSection
          ...
          <div className="flex gap-2.5 flex-1">
            {!isTicketClosed &&
              ...
              ticketId && (
                <button
                  ...
                  disabled={saving}
                  className="flex-1 px-3 py-2 text-sm bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-1 font-medium min-h-[44px] md:min-h-0"
                >
                  <Trash2 className="w-4 h-4" />
                  Delete
                </button>
            ...
            {!isTicketClosed &&
              ...
              ticketId && (
                <button
                  ...
                  disabled={saving}
                  className="flex-1 px-3 py-2 text-sm bg-yellow-500 text-white rounded hover:bg-yellow-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-1 font-medium min-h-[44px] md:min-h-0"
                >
              ...
              <>
                ...
                {ticketId && !ticket?.completed_at && canMarkCompleted && (
                  <button
                    ...
                    disabled={saving}
                    className="flex-1 px-3 py-2 text-sm bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-1 font-medium min-h-[44px] md:min-h-0"
                  >
                ...
                {ticketId && (
                  <button
                    ...
                    disabled={saving}
                    className="flex-1 px-3 py-2 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium text-center min-h-[44px] md:min-h-0"
                  >
                    Close Ticket
                  </button>
                )}
            ...
            {isTicketClosed && !canReopen && canRequestChanges && ticketId && (
              <button
                ...
                className="flex-1 px-3 py-2 text-sm bg-amber-600 text-white rounded hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium text-center min-h-[44px] md:min-h-0"
              >
        ...
        <div className="space-y-3">
          {activityLogs.length === 0 ? (
            ...
            activityLogs.map((log) => (
              <div
                key={log.id}
                className="border-l-4 border-blue-500 bg-gray-50 px-4 py-3 rounded-r-lg"
              >
                    ...
                    const filteredChanges = Object.entries(log.changes).filter(
                      ([key, value]) =>
                        ![
                          ...
                          "ticket_no",
                          "closed_by_roles",
                        ].includes(key) && shouldShowValue(value),
                    );
          ...
          <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-lg">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-700">
                You are about to permanently delete this open ticket. All ticket
                information and associated items will be removed.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700">
                The ticket will be marked as voided and excluded from all
                financial reports. This action can only be reversed by an
                administrator.
              </p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {hasExistingPaymentData() && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-start gap-2">
              ...
              <div>
                <p className="text-sm font-medium text-blue-900">
                  Previously Saved Payment Details
                </p>
                <p className="text-xs text-blue-700 mt-0.5">
                  The fields below are pre-filled with previously saved payment
                  information. You can review and modify as needed.
                </p>
              </div>
            </div>
          )}
                ...
                <div className="relative">
                  ...
                  <NumericInput
                    ...
                    onChange={(e) =>
                      setTempPaymentData({
                        ...tempPaymentData,
                        payment_gift_card: e.target.value,
                      })
                    }
                    className="pl-8 pr-3"
                    placeholder="0.00"
                      ...
                      <NumericInput
                        ...
                        onChange={(e) => {
                          ...
                          const discountAmount =
                            Math.round(((subtotal * percentage) / 100) * 100) /
                            100;
                          setTempPaymentData({
                            ...tempPaymentData,
                      ...
                      <div className="relative">
                        ...
                        <NumericInput
                          ...
                          onChange={(e) =>
                            setTempPaymentData({
                              ...tempPaymentData,
                              tip_receptionist: e.target.value,
                            })
                          }
                          disabled={
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-gray-900">
                      ${tempSubtotal.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between text-red-600">
                    <span>Total Discounts:</span>
                    <span className="font-semibold">
                      -${tempTotalDiscount.toFixed(2)}
                    </span>
                  </div>
                  {enableTax && (
                    <>
                  ...
                  {!hideTips && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        Total Tips Given by Clients:
                      </span>
                      <span className="font-semibold text-gray-900">
                        ${tempTotalTipsClient.toFixed(2)}
                      </span>
                    </div>
                  )}
