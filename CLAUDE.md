...
### 2026-02-13
- **Fix CSV import ignoring purchase unit for new sub-items**: Phase 4 of `handleCsvImport` now handles unmatched/missing purchase units instead of silently skipping them. Three cases: (1) match found — existing behavior unchanged; (2) multiplier derivable (old-format CSV with both `quantity` and `purchase_qty`) — auto-creates unit in `store_product_purchase_units` with 23505 fallback, sets `purchase_unit_id`, updates cache for subsequent rows, calculates costs; (3) multiplier not derivable (new-format CSV) — pre-fills the "Add New Purchase Unit" inline form (`isAddingPurchaseUnit=true`, `purchase_unit_id='__add_new__'`) with unit name (matched against `UNITS` constants or set as custom), leaves multiplier blank for user. Toast enhanced to warn about items needing attention (e.g. "Imported 5 items — 2 items need purchase unit multiplier"). No migration needed. Files: `InventoryTransactionModal.tsx`
