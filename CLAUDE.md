...
### Resources: "Read By" Indicator on Resource Cards (Feb 22)
- Manager-only read tracking indicator on resource cards and view modal. New `get_resource_read_summaries(p_store_id)` RPC computes per-resource read counts server-side using CTEs: CROSS JOINs active employees × active resources, filters by the 3-way AND visibility logic (visible_store_ids, visible_roles, visible_employee_ids), LEFT JOINs read_status to count reads. Returns `resource_id`, `read_count`, `total_targeted`, and `readers` JSONB array (each entry: employee_id, display_name, read_at where null = unread). Single call, no N+1. `ReadStatusIndicator` inline component in card footer shows `Users` icon + `X/Y` pill — emerald when all read, amber when partial, gray when none. Click opens upward popover (`bottom-full`) listing each targeted employee with `Check` icon (emerald, read) or `Circle` icon (gray, unread). `e.stopPropagation()` on pill + popover prevents card click. ResourceViewModal adds "Read Status" section below description with progress bar + full reader list with read dates. All gated behind `canManage` — non-managers see no pill and trigger no RPC. `readSummaries` Map state refreshed on initial load, refresh button, modal success, and mark-as-read. `ResourceReadSummary` interface added. i18n keys `readBy`/`readStatus`/`noTargetedEmployees`/`ofRead` in 4 locales. Files: `ResourcesPage.tsx`, `supabase.ts`, `i18n.ts` + migration

### Landscape Rotation Banner for Overflow Tables (Feb 21)
- Shared `LandscapeBanner` component (`src/components/ui/LandscapeBanner.tsx`) prompts phone users in portrait mode to rotate for wide data tables. Uses `window.matchMedia('(orientation: portrait)')` with `change` event listener for reactive orientation detection. Auto-hides in landscape, dismissible via X button (session-scoped state, resets on navigation). `md:hidden` ensures it never appears on tablet/desktop. Blue-50 styling with `Smartphone` + `RotateCcw` icons. Integrated above `WeeklyCalendarView` in TipReportPage (weekly + period views), TicketsPeriodView, and above the attendance table in AttendancePage. i18n key `common.rotateLandscape` added in all 4 locales. No DB changes. Files: `LandscapeBanner.tsx` (new), `TipReportPage.tsx`, `TicketsPeriodView.tsx`, `AttendancePage.tsx`, `i18n.ts`

### Mobile UX Audit: iPhone Portrait 375px for Technicians/Trainees (Feb 21)
- Comprehensive mobile responsiveness pass targeting iPhone portrait width (375–430px) for Technician/Trainee users. TicketEditor: discount grids changed from `grid-cols-2` to `grid-cols-1 xs:grid-cols-2` so fields stack vertically on narrow screens. TipReportPage: removed `min-w-[130px]` on week/period navigation labels (replaced with `whitespace-nowrap`), filter panel constrained to viewport width (`max-w-[calc(100vw-2rem)]`), all micro-typography bumped +1px (8→9, 9→10, 10→11px) for better readability on small screens. WeeklyCalendarView: date column widths reduced from 72px to 60px on mobile, added smooth touch scrolling (`-webkit-overflow-scrolling: touch`) and `scrollbar-hide` for cleaner horizontal scroll. Layout: store dropdown capped at viewport width (`max-w-[calc(100vw-120px)]`), hamburger/queue/settings/logout buttons all get `min-h-[44px]` touch targets per Apple HIG. TicketsPage mobile card view and PendingApprovalsPage dropdown tabs were already implemented — no changes needed. No DB changes. Files: `TicketEditor.tsx`, `TipReportPage.tsx`, `WeeklyCalendarView.tsx`, `Layout.tsx`

### Distribution Approvals: View, Edit, Reject (Feb 21)
- Added View, Edit, and Reject buttons to Pending Approvals → Inventory tab for distribution confirmations. **View** (Eye icon) opens `DistributionDetailModal` Drawer showing batch summary (item, employees, date, total qty/value, condition notes), dual status badges (acknowledgment + manager approval), rejection info when applicable, and lot breakdown table (distribution #, lot #, qty, unit cost, line total). **Reject** (XCircle icon, manager_approve only) opens reason modal then calls `reject_distribution` RPC which atomically reverses stock: returns quantity to `inventory_purchase_lots` (sets lot back to `active`), deletes `employee_inventory_lots` entries, decrements `employee_inventory.quantity_on_hand`, restores `store_inventory_levels`. Only `pending`/`acknowledged` rows get stock reversal — `in_use`/`returned`/`consumed` rows are marked rejected but stock is not reversed (returned in `skipped_count`). Same hierarchy check as approve (rejector must outrank creator). Self-rejection blocked. **Edit** (Pencil icon, manager_approve only) rejects old distribution with reason "Replaced by new distribution" then opens `EmployeeDistributionModal` for fresh creation. `get_distribution_batch_details` RPC returns per-lot details with all employee names and approval/rejection metadata. Updated `get_pending_distribution_approvals` to exclude `rejected`/`cancelled` rows from manager approval query. New columns on `inventory_distributions`: `rejection_reason`, `rejected_by_id`, `rejected_at`. Status CHECK updated to include `'rejected'`. `DistributionBatchDetail` TypeScript interface added. Files: `PendingApprovalsPage.tsx`, `DistributionDetailModal.tsx` (new), `supabase.ts` + migration

### Commission Tier-Based Validation + Guide Modal (Feb 21)
- Tier-based validation on the Commission field in Payment Details modal, paralleling the receptionist tip paired system but operating on add-on prices. Per-item tier formula: addon <$20 → $0 (field blocked), $20–$29.99 → $5, $30+ → $10. Multi-service tickets sum each item's tier max. Cascading customer tip allocation: customer tips reduce the tip paired allowance first; only overflow past the tip tier max reduces commission allowance. Three layers of enforcement: (1) `NumericInput` `max` attribute limits input to allowance, (2) three-state helper text ("No add-ons qualify..." / "Customer tip exceeds..." / "Max $X.XX"), (3) `handlePaymentModalSave()` blocks with error toast if exceeded (0.005 epsilon). When commission allowance drops to 0, field is disabled and auto-cleared. Customer tip cash/card onChange handlers reactively clear `tip_commission` when commission allowance drops to 0. `getCommissionTierMax()` pure helper computes tier sum from items. `getMaxCommissionTip()` returns `{tierMax, allowance}` using cascading overflow logic. HelpCircle "?" button on Commission label opens Commission Guide modal with tier table, combined allowance formula, 4 worked examples, and note about Commission Paired employee checkbox. No DB changes. Files: `TicketEditor.tsx`

### Receptionist Paired Tip: Tier-Based Maximum Validation (Feb 21)
- Tier-based maximum validation on receptionist paired tips in Payment Details modal. Per-item tier formula: base price ≤$20 → $2 max, ≤$40 → $3, >$40 → $5. Multi-service tickets sum each item's tier max. Customer tips (cash + card) reduce the allowance — remaining amount caps the paired tip field. Three layers of enforcement: (1) `NumericInput` `max` attribute limits input to allowance, (2) helper text below field shows "Max $X.XX" or "Customer tip meets or exceeds the maximum ($X)" when exhausted, (3) `handlePaymentModalSave()` blocks with error toast if somehow exceeded. When allowance drops to 0, paired tip field is disabled (greyed out) and auto-cleared. Customer tip cash/card onChange handlers reactively clear paired tip when new allowance ≤ 0. `getMaxReceptionistTip()` helper computes `tierMax` and `allowance` from `items` and `tempPaymentData`. 0.005 epsilon for floating-point tolerance. No DB changes. Files: `TicketEditor.tsx`
...
- New "Commission" input field in TicketEditor Payment Details modal, positioned as a fourth column in the Tips section (right of "Tip Paired by Receptionist"). Commission is always editable (disabled only when ticket is closed/read-only) — decoupled from `tip_paired_enabled` flag which only gates the "Tip Paired by Receptionist" field. Grid layout: `md:grid-cols-3` for Cash/Card (3 fields), `md:grid-cols-2` for Mixed (4 fields wrap 2x2). Commission shown separately below Grand Total in Payment Summary (purple text, only when > 0). New `tip_commission` numeric(10,2) column on `ticket_items` (DEFAULT 0.00). Stored only on first ticket item (same pattern as all other tip fields). `TicketItem` interface, `TicketItemForm` interface, `formData`, `tempPaymentData`, and all item initialization/save/load sites updated. Tip Report daily view: purple "Commission" row after T. (paired) in Daily Total, multi-service ticket tips, and single-service ticket tips. Only shown when commission > 0. Commission included in Total calculations. `ServiceItemDetail`, `TechnicianSummary`, `TicketGroup` interfaces extended with `tip_commission`/`tips_commission`/`totalTipCommission`. Weekly/period views: purple commission row in `WeeklyCalendarView` for both daily cells and Period Total column. `WeeklyDataTips` type extended. Weekly and period data aggregation functions both updated with `tips_commission` tracking through store breakdown maps. Files: `TicketEditor.tsx`, `TipReportPage.tsx`, `WeeklyCalendarView.tsx`, `supabase.ts` + migration
- Fix commission missing from weekly/period Tip Report summary cards. `technicianMap` construction in `fetchWeeklyData` and `fetchPeriodData` accumulated `totalCustomer` and `totalReceptionist` but omitted `totalCommission`. Grid cells displayed correctly (reading from raw `weeklyData` Map) while sidebar summaries showed zero commission. Added `totalCommission` accumulator, `tips_commission` field, and included commission in `tips_total` for both functions. No DB changes. Files: `TipReportPage.tsx`
- Always display Commission row in Tip Report Daily view regardless of value. Removed `> 0` conditional wrappers from three rendering locations: technician summary cards, multi-service ticket tips, and single-service ticket tips. Commission row now renders unconditionally (showing 0 when no commission data), matching T. (given) and T. (paired) which always display. No DB changes. Files: `TipReportPage.tsx`
...
### Inventory Approvals: Edit Pending Transactions (Feb 20)
- Edit button on Pending Approvals → Inventory tab for IN and OUT transactions (transfers keep existing Review flow). Pencil icon button positioned between Approve and Reject, gated by `canTakeActions` (management only). No self-edit guard — managers may edit their own transactions. `handleEditInventory` handler fetches transaction header (`supplier_id`, `invoice_reference`, `recipient_id`, `notes`) and items (`inventory_transaction_items`) to construct `DraftTransaction` shape. Opens `InventoryTransactionModal` with new `pendingTransaction` prop. New `isEditingPending` boolean state in modal controls behavior: title shows "Edit Pending Transaction", footer shows single "Save Changes" button (hides Save Draft / Delete Draft / Submit for Approval), transaction type selector disabled, helper text updated. Save flow calls new `update_pending_transaction` RPC (identical to `update_draft_transaction` but with `status = 'pending'` guard), deletes existing items and re-inserts via `insert_transaction_items_batch`. Transaction stays pending after edit — approval is a separate action. Second `useEffect` for purchase unit/brand population extended to trigger on `pendingTransaction`. Files: `InventoryTransactionModal.tsx`, `PendingApprovalsPage.tsx` + migration
- Fix `get_historical_inventory_approvals` RPC 400 error: two column reference bugs — `iti.inventory_transaction_id` (correct: `iti.transaction_id`) in the LEFT JOIN, and `SUM(iti.total_cost)` (correct: `SUM(iti.unit_cost * iti.quantity)`) for total value calculation. Both columns don't exist on `inventory_transaction_items`. Fixed in single migration. Migration only
